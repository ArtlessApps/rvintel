export const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL ?? "https://www.rvintel.io";

export const SITE_NAME = "RVIntel";

export const SITE_DESCRIPTION =
  "RV rental market intelligence across 35 US markets. RVIntel tracks competitor rates, occupancy signals, and pricing trends so hosts know exactly what their rig should earn.";

/** Relative path — resolved via metadataBase for OG/Twitter tags. */
export const DEFAULT_OG_IMAGE = "/images/Dashboard.png";

export const DEFAULT_OG_IMAGE_ALT =
  "RVIntel Market Dashboard showing average market rate, rate distribution, and rate trend over time";
