#!/usr/bin/env node
// Outdoorsy ArkLaTex backfill — direct JSON:API (2026-06-06).
//
// Mirrors backfill_outdoorsy_sd.mjs exactly — only MARKET and ADDRESS differ.
// Search anchored to Shreveport, LA (Shreveport–Bossier metro core).
//
// Usage:
//   node scripts/backfill_outdoorsy_arklatex.mjs
...
// ── config ────────────────────────────────────────────────────────────────────
const MARKET = "arklatex";
const PLATFORM = "outdoorsy";
const ADDRESS = "Shreveport, LA";
const PAGE_SIZE = 24;
...
async function main() {
  ...
  console.log(`Outdoorsy ArkLaTex backfill (direct API) started ${startedAt.toISOString()}`);
